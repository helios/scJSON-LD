...
labels=geneIDs() #SPARQL query EBI-RDF
...
myJSONLD<-paste('
{
  ...
  "data":[',
  jsonData(
    data=jsonMatrix("https://s3-us-west-2.amazonaws.com/10x.files/samples/cell/pbmc3k/pbmc3k_filtered_gene_bc_matrices.tar.gz","http://edamontology.org/data_3273","10XCellBarcoding", columnType, columnLabel),
  ...
  "name": "Seurat",
  "input": { "@id": "http://mock_schema.org/1.1"}
  }
  }', sep='')



data10x <- Read10X("./filtered_gene_bc_matrices/hg19/")
dataSeurat <- CreateSeuratObject(raw.data = data10x, min.cells = 3, min.genes = 200, project = "10xMyExp")
```
```{r, echo=FALSE, results='asis'}
cat(bhjsonldHtmlScript(myJSONLD))
```
```{r}
